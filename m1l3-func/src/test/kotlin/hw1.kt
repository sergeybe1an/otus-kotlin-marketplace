...
class HomeWork1Test {
    ...
    private fun mapListToNames(list: List<Map<String, String>>): List<String> =
        list.map { buildFullName(it["last"], it.values) }

    private fun buildFullName(last: String? = "", parts: Collection<String>): String =
        (if (!last.isNullOrEmpty()) "$last " else "") +
                parts.filter { it != last }.joinToString(" ")
